...
fun test_calculate_tokens_to_mint() {
    ...
    assert!(tokens_to_mint == 182, E_WRONG_TOKENS_TO_MINT);
}
...
fun test_calculate_sale_return() {
    ...
    let sale_return = bonding_curve::calculate_sale_return(
        virtual_sui,
        virtual_tokens,
        token_amount,
    );
    ...
    assert!(sale_return == 48, E_WRONG_SALE_RETURN);
}
...
fun test_tokens_and_sale_impact() {
    ...
    // Buy tokens
    let tokens_minted = bonding_curve::calculate_tokens_to_mint(
        virtual_sui,
        virtual_tokens,
        sui_to_spend,
    );
    ...
    assert!(tokens_minted > 0, 0);

    // Sell a smaller amount back (half of what we got)
    let tokens_to_sell = tokens_minted / 2;
    let sui_returned = bonding_curve::calculate_sale_return(
        new_virtual_sui,
        new_virtual_tokens,
        tokens_to_sell,
    );
    ...
    let buy_price_per_token = sui_to_spend / tokens_minted;
    let sell_price_per_token = sui_returned / tokens_to_sell;

    // Sell price per token should be less than or equal to buy price per token
    // due to the way bonding curve mechanics work
    assert!(sell_price_per_token <= buy_price_per_token, E_WRONG_SALE_RETURN);
}
